...
const DAMPING_MIN = 50;
const DAMPING_MAX = 600;
...
function normalizeDamping(value) {
  const damping = Number(value);
  if (!Number.isFinite(damping)) return DAMPING_MIN;
  return Math.min(DAMPING_MAX, Math.max(DAMPING_MIN, Math.round(damping)));
}
...
function updateFieldValue(field, value) {
  if (field === "dampingFactor") {
    value = normalizeDamping(value);
  }
...
function parseNotifyPacket(bytes) {
  ...
  if (cmd === 0x81 && bytes.length >= 11) {
    ...
    const damping = normalizeDamping((bytes[3] << 8) | bytes[4]);
...
async function writeParams() {
  ...
  const damping = normalizeDamping(state.values.dampingFactor);
  updateFieldValue("dampingFactor", damping);
